...
export async function siteConfig(req) {
  ...
  const value = {
    factory: valid(process.env.PLEDGE_FACTORY) || pick('PLEDGE_FACTORY'),
    burner: valid(process.env.PLEDGE_BURNER) || pick('PLEDGE_BURNER'),
    ca: valid(process.env.PLEDGE_CA) || ca,
    origin
  };
