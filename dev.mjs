...
if (process.argv.includes('--memory')) process.env.PLEDGE_MEMORY = '1';
...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (/^\/p\/0x[0-9a-fA-F]{40}\/?$/.test(p)) p = "/pledge.html";
...
}).listen(Number(process.env.PORT || 8800), () => console.log('pledge dev on', process.env.PORT || 8800));
